...
const StudySpace = () => {
  ...
  // ❗ MOVE THIS HOOK TO TOP
  const [joinSpaceState, setJoinSpaceState] = useState({
    loading: false,
    error: null,
    success: false
  });
  ...
  const getInviteCode = async (spaceId) => {
    try {
      setInviteCodeState(prev => ({
        ...prev,
        loading: true
      }));
      const res = await axiosInstance(`/study-space/getInvitecode/${spaceId}`)
      console.log("invite COde res?.data?.data?.inviteCode  : ", res?.data?.data?.inviteCode)
      setInviteCodeState(prev => ({
        ...prev,
        data: res?.data?.data?.inviteCode,
        error: null
      }));
    } catch (error) {
      let msg = extractErrorMessage(error)
      setInviteCodeState((state) => ({
        ...state,
        data: null,
        error: msg,
      }))
  ...
  if (!spaceId || !authUser?.id || !isInitialized || status?.checkStudySpace?.loading) {
    return <StudySpaceLoadingSpinner mode={mode} />;
  }

  const joinSpace = async () => {
    if (!inViteCodeState) {

      setJoinSpaceState(prev => ({
        ...prev,
        loading: false,
    ...
    try {
      ...
      await axiosInstance.post('/study-space/join', {
        inviteCode: inViteCodeState?.data,
      });
      setJoinSpaceState({
        error: null,
        loading: false,
  ...
  if (status?.checkStudySpace?.error) {
    return (
      <div className={`w-full h-screen flex items-center justify-center ${bgPrimary}`}>
        <StudySpaceError error={status.checkStudySpace.error} joinSpace={joinSpace} joinSpaceState={joinSpaceState} />
      </div>
    );
  }
