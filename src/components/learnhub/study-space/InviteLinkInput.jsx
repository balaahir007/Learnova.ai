...
const InviteLinkInput = ({ inviteCode }) => {
  ...
  const handleCopy = () => {
    navigator.clipboard
      .writeText(`https://Learnova.ai.onrender.com/study-space/join/${inviteLink}`)
      .then(() => {
        setCopied(true);
  ...
  return (
    <div className="flex items-center gap-2 p-4 w-full max-w-xl bg-backgray rounded-xl shadow-sm border border-gray-200">
      <input
        ref={inputRef}
        type="text"
        value={inviteLink}
        readOnly
        className="flex-1 px-3 py-2 rounded-md bg-backgray border border-gray-300 text-sm text-primary focus:outline-none focus:ring-2 focus:ring-blue-500"
        style={{
          overflowX: 'auto',
          whiteSpace: 'nowrap',
        }}
