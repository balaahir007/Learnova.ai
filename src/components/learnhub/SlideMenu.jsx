...
const SlideMenu = ({ extendMenuOptions, setExtendMenuOptions, isInterviewGoing, isCourseDetailPage, mode = 'dark' }) => {
  ...
  return (
    <>
      ...
      <div 
        className={`block md:hidden w-full fixed bottom-0 left-0 right-0 z-[9999] ${navBg} ${mode === 'dark' ? 'border-t-2 border-gray-700' : 'border-t-2 border-gray-300'} ${mode === 'dark' ? 'shadow-[0_-2px_10px_rgba(0,0,0,0.5)]' : 'shadow-[0_-2px_10px_rgba(0,0,0,0.1)]'} transition-transform duration-300 ${showBottomBar ? 'translate-y-0' : 'translate-y-full'}`}
        style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}
      >
        <div className="flex justify-around items-center px-1 py-4 max-w-screen-xl mx-auto">
